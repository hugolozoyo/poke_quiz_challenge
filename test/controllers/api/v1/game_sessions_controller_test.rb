  ...
  module V1
    class GameSessionsControllerTest < ActionDispatch::IntegrationTest
      ...
      test 'a game session from index contains the expected data' do
        ...
        assert_response :success
        assert_equal %w[id username score started_at finished_at attempt_count created_at updated_at].sort, game_session.keys.sort
      end
      ...
      test 'a game session from show contains the expected data' do
        ...
        assert_response :success
        assert_equal %w[id username score started_at finished_at attempt_count created_at updated_at].sort, body.keys.sort
      end
