...
class GameSessionTest < ActiveSupport::TestCase
  ...
  test 'numericality validations for attempt_count' do
    game_session = GameSession.new(attempt_count: 'a')

    assert game_session.invalid?
    assert_model_validation_error game_session, attribute: :attempt_count, error: :not_a_number
  end
  ...
  test 'numericality validations for attempt_count greater than or equal to zero' do
    game_session = GameSession.new(attempt_count: -1)

    assert game_session.invalid?
    assert_model_validation_error game_session, attribute: :attempt_count, error: :greater_than_or_equal_to
  end
  ...
  test 'retry! increments attempt_count by one, sets score to zero, and sets finished_at to nil' do
    game_session = game_sessions(:hugobelman)

    travel_to Time.current do
      game_session.retry!

      assert_equal Time.current, game_session.started_at
      assert_equal 2, game_session.attempt_count
